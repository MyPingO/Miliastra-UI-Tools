...
export function parse(bytes) {
  ...
  while (p < bytes.length) {
    ...
    if (wire === 0) {
      ...
      p += wire === 1 ? 8 : 4;
      value = bytes.subarray(valueStart, p);
    } else if (wire === 2) {
      const size = readVarint(bytes, p);
      if (size.value > BigInt(bytes.length - size.offset))
        throw new Error('Field extends beyond its message.');
      valueStart = size.offset;
      p = valueStart + Number(size.value);
      value = bytes.subarray(valueStart, p);
    ...
    fields.push({
      ...
      occurrence,
      raw: bytes.subarray(start, p),
    });
  }
  return fields;
}
export const all = (bytes, n, wire) =>
  parse(bytes).filter((f) => f.number === n && (wire === undefined || f.wire === wire));
...
export const text = (bytes, n, fallback = '') => {
  const f = one(bytes, n, 2);
  return f ? string(f.value) : fallback;
};
export function field(n, wire, value) {
  ...
  if ((wire === 1 && value.length !== 8) || (wire === 5 && value.length !== 4))
    throw new Error('Wrong fixed-width size.');
...
export function set(bytes, n, wire, value) {
  ...
  return target
    ? concat([bytes.subarray(0, target.start), replacement, bytes.subarray(target.end)])
    : concat([bytes, replacement]);
}
...
export function patch(bytes, path, replacement) {
  ...
  if (!f || f.wire !== 2) throw new Error('Stored path no longer matches the file.');
  return concat([
    bytes.subarray(0, f.start),
    field(n, 2, patch(f.value, path.slice(1), replacement)),
    bytes.subarray(f.end),
  ]);
}
...
export function unpack(bytes) {
  if (!(bytes instanceof Uint8Array) || bytes.length < 24)
    throw new Error('A GIL/GIA file needs a 20-byte header and 4-byte footer.');
  if (bytes.length > 256 * 1024 * 1024)
    throw new Error('Files larger than 256 MB are not supported.');
  const view = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
  const size = view.getUint32(0),
    version = view.getUint32(4),
    magic = view.getUint32(8),
    type = view.getUint32(12),
    length = view.getUint32(16);
  if (magic !== 0x326 || view.getUint32(bytes.length - 4) !== 0x679)
    throw new Error('Invalid Miliastra header or footer magic.');
  if (size !== bytes.length - 4 || length !== bytes.length - 24)
    throw new Error('Header lengths do not match this file.');
...
export function pack(payload, type = 2, version = 1) {
  const header = new Uint8Array(20),
    footer = new Uint8Array(4),
    view = new DataView(header.buffer);
  [payload.length + 20, version, 0x326, type, payload.length].forEach((v, i) =>
...
export function floatBytes(value) {
  ...
  if (!Number.isFinite(view.getFloat32(0, true)))
    throw new Error('Value exceeds the Float32 range.');
  return bytes;
}
